...
def extract_spectrum_og(image_path, master_bias, master_flat, crop, master_comp, mjd, location, ra, dec, compparams=None, modechoice="SOAR_930"):
    if os.name == "nt":
...
def extract_spectrum(image_path, master_bias, master_flat, crop, master_comp, mjd, location, ra, dec, comp_header, compparams=None):
    ...
    if compparams is None:
        ...
        fitlines.argtypes = [ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.POINTER(ctypes.c_double), ctypes.c_int, ctypes.c_int, ctypes.c_double, ctypes.c_double]
        fitlines.restype = ctypes.POINTER(ctypes.c_double)

        def call_fitlines(compspec_x, compspec_y, center, extent):
            ...
            result_ptr = fitlines(compspec_x, compspec_y, lines, lines_size, compspec_size, center, extent)
        ...
        result = call_fitlines(pixel, compflux, central_wl, 1700)
        print(result)
